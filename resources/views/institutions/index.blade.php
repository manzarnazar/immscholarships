...
<div class="col-12 flex justify-between items-center">
    <h2 class="intro-y text-lg font-medium mt-10">All Registered Institutions</h2>
    <div class="flex items-center">
        <a href="{{ route('admin-institutions-create') }}" class="btn btn-success">
            <i class="align-middle me-2" data-feather="plus"></i> Add University
        </a>
    </div>
</div>
        ...
        <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
            <table class="table table-report -mt-2">
                ...
                <tbody id="course-data">
                    @include('institutions.partials._table_data')
                </tbody>
            </table>
        </div>
        <!-- END: Data List -->
        ...
        <div class="intro-y col-span-12 flex flex-wrap sm:flex-row sm:flex-nowrap items-center">
            <nav class="w-full sm:w-auto sm:mr-auto">
                <ul class="pagination" id="pagination">
                    @include('layout.components.pagination', ['paginator' => $institutions])
                </ul>
            </nav>
...
$(document).ready(function() {
    ...
    function fetchInstitutionData(page = 1) {
        let search = $('#search').val();
        let entriesPerPage = $('#entries-per-page').val();
        $.ajax({
            url: `{{ url('dashboard/admin/institutions') }}`,
            type: 'GET',
            data: {
                search: search,
