...
class MasterEducationController extends Controller
{
    ...
    {
        $degree = MasterEducation::where("user_id", auth()->user()->id)->paginate(10);
        return view("master.index", compact("degree"));
    }
    ...
    public function fetchDegrees(Request $request)
{
    ...
    $degree = MasterEducation::where('user_id',auth()->user()->id)
    ...
    $view = view('master.master-data', compact('degree'))->render();
$pagination = view('layout.components.pagination', ['paginator' => $degree])->render();
    ...
    public function create(Request $request)
    {
        ...
        $existingDegreeEducation = MasterEducation::where('user_id', $userId)->first();

        if ($existingDegreeEducation) {
            Alert::toast('Education Master already exists for this user','error');
            return redirect()->route('master-education');
        }
        return view("master.details");
    }
    ...
    public function store(Request $request)
    {
        ...
        $user_id = auth()->id();
        $degreeEducation = new MasterEducation;
        ...
        return redirect()->route('master-education')->with('success', 'Master Education Information Added!');
    }

    /**
     * Display the specified resource.
     */
    public function show(MasterEducation $degree)
    {
        return view('master.details', compact('degree'));
    }

    /**
     * Show the form for editing the specified resource.
     */
    public function edit(MasterEducation $degreeEducation)
    {
        //
    }
     ...
     */
    public function update(Request $request, MasterEducation $degreeEducation)
    {
        $commonRules = [
            'start_date' => 'required|date',
            'end_date' => 'required|date',
        ...
        return redirect()->route('master-education')->with('success', 'Degree Education Information Updated!');
    }

    /**
     * Remove the specified resource from storage.
     */
    public function destroy(MasterEducation $degreeEducation)
    {
        //
    }
}
